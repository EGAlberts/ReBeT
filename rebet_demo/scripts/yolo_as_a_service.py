    ...
    def detect_obj_service(self, request, response):
        ...
        for i in range(num_preds):
            ...
            if rgb_msg is not None:
                ...
                if(len(results) == 0):
                    self.publisher_.publish(rgb_msg)
                for i, r in enumerate(results):
                    # Plot results image
                    self.publisher_.publish(self.bridge.cv2_to_imgmsg(r.plot()))
