...
#include "rebet_msgs/srv/register_new_tree.hpp"
...
public:
  ...
  using RegisterNewTree = rebet_msgs::srv::RegisterNewTree;
  ...
  Arborist(const rclcpp::NodeOptions & options)
  : TreeExecutionServer(options)
  {
    _set_param_in_blackboard = node()->create_service<SetParameterInBlackboard>(
      "set_parameter_in_blackboard", std::bind(&Arborist::handle_set_param_bb, this, _1, _2));
    _set_att_in_blackboard = node()->create_service<SetAttributesInBlackboard>(
      "set_attributes_in_blackboard", std::bind(&Arborist::handle_set_atb_bb, this, _1, _2));
    _get_blackboard =
      node()->create_service<GetBlackboard>(
      "get_blackboard",
      std::bind(&Arborist::handle_get_bb, this, _1, _2));
    _reqister_new_tree = node()->create_service<RegisterNewTree>(
      "register_new_tree",
      std::bind(&Arborist::handle_new_tree, this, _1, _2));
    node()->create_service<GetQR>("get_qr", std::bind(&Arborist::handle_get_qr, this, _1, _2));
...
protected:
  ...
  void handle_get_bb(
    const std::shared_ptr<GetBlackboard::Request> request,
    ...
    std::shared_ptr<GetQR::Response> response)
  {
    ...
    std::cout << "End service to get a QR nodes from the blackboard " <<
      response->qrs_in_tree.size() << std::endl;

  }

  void handle_new_tree(
    const std::shared_ptr<RegisterNewTree::Request> request,
    std::shared_ptr<RegisterNewTree::Response> response)
  {
    RCLCPP_INFO(kLogger, "Service to get a Register new tree through a ros2 service");
    try {
      factory().registerBehaviorTreeFromText(request->tree_xml);
      RCLCPP_INFO(kLogger, "Succesfully registered new BT through ROS2 Service");
      response->success = true;

    } catch (const std::exception & e) {
      RCLCPP_ERROR(
        kLogger, "Failed to register BT: %s \n %s",
        request->tree_xml.c_str(), e.what());
      response->success = false;
    }
  ...
  rclcpp::Service<RegisterNewTree>::SharedPtr _reqister_new_tree;
