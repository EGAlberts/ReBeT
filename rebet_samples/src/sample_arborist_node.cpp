...
{
public:
  SleepAndReport(const std::string & name, const NodeConfig & config)
  : SleepNode(name, config)
  {
    _sleep_count = 0;
  }
  ...
  static PortsList providedPorts()
  {
    ...
    PortsList child_ports = {OutputPort<int>(
        "sleep_count",
        "How long the system has slept, in seconds"), };
...
{
public:
  DontSleepTooLong(const std::string & name, const NodeConfig & config)
  : TaskLevelQR(name, config, QualityAttribute::Test)
  {}
  ...
  static PortsList providedPorts()
  {
    ...
    PortsList child_ports = {InputPort<int>(
        "sleep_count",
        "How long the system has slept, in seconds"), };
    child_ports.merge(base_ports);
...
{
public:
  SampleArborist(const rclcpp::NodeOptions & options)
  : Arborist(options)
  {
