...
enum class AdaptationType {External, Internal, Tactical};
...
{
  if (str == "External") {
    return AdaptationType::External;
  }
  if (str == "Internal") {
    return AdaptationType::Internal;
  }
  if (str == "Tactical") {
    return AdaptationType::Tactical;
  }

  throw RuntimeError(
          std::string("Cannot convert this to AdaptationType: ") +
          static_cast<std::string>(str));
}

template<>
std::string toStr<AdaptationType>(const AdaptationType & type)
{
  switch (type) {
    case AdaptationType::Internal:
      return "Internal";
    case AdaptationType::External:
      return "External";
    case AdaptationType::Tactical:
      return "Tactical";
    default:
      return "Undefined";
  }
...
protected:
  ...
  internal_future_response_;
  std::shared_future<aal_msgs::srv::AdaptArchitectureTactical::Response::SharedPtr>
  tactical_future_response_;
  ...
  aal_msgs::srv::AdaptArchitectureTactical::Response::SharedPtr tactical_response_;
  ...
  std::chrono::milliseconds service_timeout_ = std::chrono::milliseconds(
    ADAP_SERVICE_TIMEOUT_MILLISECOND);
  ...
  void sendAdaptationRequest(
    const std::string & service_name,
    typename rclcpp::Client<AdaptationService>::SharedPtr & client,
    const std::string & registration_name, const std::string & strategy_name = "")
  {
    client = node_->create_client<AdaptationService>(
    ...
    if constexpr (std::is_same_v<AdaptationService, aal_msgs::srv::AdaptArchitectureExternal>) {
      ...
      std::cout << "internal adap sent type " <<
        _internal_adaptations[0].parameter_adaptation.value.type << std::endl;
    ...
    if (ret != rclcpp::FutureReturnCode::SUCCESS) {
      if ( (node_->now() - time_request_sent_) > timeout) {
        throw std::runtime_error(
                "ran out of time trying to request adaptation, is your adaptation logic working properly? ");
      } else {
        return false;
      }
    } else {
        ...
        aal_msgs::srv::AdaptArchitectureTactical>)
      {
        ...
        tactical_response_ = tactical_future_response_.get();
        std::cout << "Got response from future!" << std::endl;
...
{
public:
  AdaptNode(const std::string & name, const NodeConfig & config)
  ...
  static PortsList providedPorts()
  {
    PortsList ports = {
      InputPort<std::string>(
        ADAP_STRAT,
        "Which strategy should be employed to decide on adaptations"),
...
class AdaptOnConditionAny : public AdaptNode, public virtual AdaptDecoratorBase
{
public:
  AdaptOnConditionAny(const std::string & name, const NodeConfig & config)
  ...
  static PortsList providedPorts()
  {
    ...
    PortsList child_ports = {
      InputPort<NodeStatus>("child_status",
                            "status of child which should trigger adaptation"),
  ...
  void sendAdaptationRequests() {
    if (period_elapsed()) {
      request_sent_ = true;
      switch (input_adaptation_type_) {
        case AdaptationType::Internal:
          {
            sendAdaptationRequest<aal_msgs::srv::AdaptArchitecture,
              aal_msgs::srv::AdaptArchitecture::Request>(
              "/adapt_architecture", internal_adapt_client_,
        ...
        case AdaptationType::External:
          {
            ...
            sendAdaptationRequest<aal_msgs::srv::AdaptArchitectureExternal,
              aal_msgs::srv::AdaptArchitectureExternal::Request>(
              "/adapt_architecture_external", external_adapt_client_,
              this->registrationName(), strategy_name);
            break;
          }
        case AdaptationType::Tactical:
          {
            this->get_leaf();
            sendAdaptationRequest<aal_msgs::srv::AdaptArchitectureTactical,
              aal_msgs::srv::AdaptArchitectureTactical::Request>(
              "/adapt_architecture_tactical", tactical_adapt_client_,
              this->registrationName());
            break;
          }
        default:
  ...
  NodeStatus tick() override {
    ...
    switch (this->status()) {
      case NodeStatus::IDLE:
        ...
        if (chosen_child_status_ == NodeStatus::IDLE) {
      ...
      case NodeStatus::RUNNING:
        ...
        if (request_sent_) {
            ...
            switch (input_adaptation_type_) {
              case AdaptationType::Internal:
                ret = callback_group_executor_.spin_until_future_complete(
                  internal_future_response_,
                  nodelay);
                receiveAdaptationRequest<aal_msgs::srv::AdaptArchitecture>(ret);
                break;
              case AdaptationType::External:
                ret = callback_group_executor_.spin_until_future_complete(
                  external_future_response_,
                  nodelay);
                receiveAdaptationRequest<aal_msgs::srv::AdaptArchitectureExternal>(ret);
                break;
              case AdaptationType::Tactical:
                ret = callback_group_executor_.spin_until_future_complete(
                  tactical_future_response_,
                  nodelay);
                receiveAdaptationRequest<aal_msgs::srv::AdaptArchitectureTactical>(ret);
                break;
            }
          }
          if (response_received_) {
            ...
            if (chosen_child_status_ == NodeStatus::SUCCESS ||
              chosen_child_status_ == NodeStatus::FAILURE)                                                   //Since the child is done, preempt ticking it and just finish.
            {
              return chosen_child_status_;
            }
          ...
          if (chosen_child_status_ == NodeStatus::IDLE) { //Response must have been received, so now we just tick until completion.
            const NodeStatus child_status = this->child_node_->executeTick();
...
class AdaptOnCondition : public AdaptNode, public virtual AdaptDecoratorBase
{
public:
  AdaptOnCondition(const std::string & name, const NodeConfig & config)
  : AdaptNode(name, config)
  {
...
{
public:
  AdaptOnConditionOnStart(
    const std::string & name, const NodeConfig & config,
    const AdaptationTarget & adaptation_target,
    ...
    if (this->status() == NodeStatus::IDLE) {
      ...
      if (_condition_was_true) {
        ...
        if (adaptation_type_ == AdaptationType::External) {
          ...
          sendAdaptationRequest<aal_msgs::srv::AdaptArchitecture,
            aal_msgs::srv::AdaptArchitecture::Request>(
            "/adapt_architecture", internal_adapt_client_, this->registrationName());
        } else if (adaptation_type_ == AdaptationType::Tactical) {
          this->get_leaf();
          sendAdaptationRequest<aal_msgs::srv::AdaptArchitectureTactical,
            aal_msgs::srv::AdaptArchitectureTactical::Request>(
            "/adapt_architecture_tactical", tactical_adapt_client_, this->registrationName());
      ...
      if (!response_received_) {
        ...
        if (adaptation_type_ == AdaptationType::External) {
          ...
          ret = callback_group_executor_.spin_until_future_complete(
            tactical_future_response_,
            nodelay);
          received = receiveAdaptationRequest<aal_msgs::srv::AdaptArchitectureTactical>(ret);
        }
        ...
        case NodeStatus::SUCCESS: {
            ...
            if (_condition_was_true && !request_sent_) {
              ...
              if (!response_received_) {
                ...
                if (adaptation_type_ == AdaptationType::External) {
                  ret = callback_group_executor_.spin_until_future_complete(
                    external_future_response_, nodelay);
                  received =
                    receiveAdaptationRequest<aal_msgs::srv::AdaptArchitectureExternal>(ret);
                } else if (adaptation_type_ == AdaptationType::Internal) {
                  ret = callback_group_executor_.spin_until_future_complete(
                    internal_future_response_, nodelay);
                  received = receiveAdaptationRequest<aal_msgs::srv::AdaptArchitecture>(ret);
                } else if (adaptation_type_ == AdaptationType::Tactical) {
                  ret = callback_group_executor_.spin_until_future_complete(
                    tactical_future_response_, nodelay);
                  received =
                    receiveAdaptationRequest<aal_msgs::srv::AdaptArchitectureTactical>(ret);
                }
        ...
        case NodeStatus::FAILURE: {
            ...
            if (_condition_was_true && !request_sent_) {
              ...
              if (!response_received_) {
                ...
                auto const nodelay = std::chrono::milliseconds(0);
                auto const timeout = rclcpp::Duration::from_seconds(
                  double(service_timeout_.count()) / 1000);
...
template<class ParamT>
class AdaptPeriodicallyOnRunning : public AdaptOnConditionOnRunning<ParamT>,
  public virtual AdaptDecoratorBase
{
  static constexpr const char * WINDOW_LEN = "adaptation_period";

public:
  AdaptPeriodicallyOnRunning(
    const std::string & name, const NodeConfig & config,
    const AdaptationTarget & adaptation_target,
    AdaptationType adaptation_type)
  : AdaptOnConditionOnRunning<ParamT>(name, config, adaptation_target, adaptation_type)
  {
    std::cout << "\n\n\n\nSomeone created me a AdaptPeriodicallyOnRunning node!!!!\n\n\n\n\n" <<
      std::endl;
    auto curr_time_pointer = std::chrono::system_clock::now();
    _window_start = std::chrono::duration_cast<std::chrono::seconds>(
      curr_time_pointer.time_since_epoch()).count();
  ...
  static PortsList providedPorts()
  {
    ...
    PortsList child_ports = {
      InputPort<int>(
        WINDOW_LEN,
        "The periodicity with which the adaptation should occur during running.")
    };
    child_ports.merge(base_ports);
